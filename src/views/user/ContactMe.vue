<template>
  <div class="contact-me">
    <div class="page-banner banner-contactme">
      <div class="banner-content"><h1 class="banner-title">联系我的</h1><p class="banner-subtitle">查看别人对你发布信息的留言</p></div>
    </div>

    <div class="search-section">
      <el-form :inline="true" :model="formInline" class="search-form">
        <el-form-item><el-input v-model="formInline.title" placeholder="搜索标题..." clearable prefix-icon="el-icon-search" size="medium" class="search-input" /></el-form-item>
        <el-form-item><el-date-picker v-model="formInline.date" value-format="yyyy-MM-dd" type="date" placeholder="选择日期" size="medium" /></el-form-item>
        <el-form-item><el-button type="primary" size="medium" :loading="loading" @click="handleClick">查询</el-button><el-button size="medium" @click="handleReset">重置</el-button></el-form-item>
      </el-form>
    </div>

    <el-tabs v-model="activeName" style="margin-bottom: 16px;" @tab-click="handleClick">
      <el-tab-pane label="失物留言" name="first" />
      <el-tab-pane label="招领留言" name="second" />
    </el-tabs>

    <div class="toolbar"><span class="result-count">共 <b>{{ total }}</b> 条留言</span></div>

    <div v-loading="loading" class="contact-list">
      <div v-if="errorMessage" class="error-box"><i class="el-icon-warning" /><span>{{ errorMessage }}</span></div>
      <div v-else-if="!loading && tableData.length === 0" class="empty-box"><i class="el-icon-chat-round" /><p>暂无留言</p></div>
      <div v-else>
        <div v-for="item in tableData" :key="item.id" class="contact-card">
          ...
          <div class="contact-body">
            ...
            <p class="contact-meta"><i class="el-icon-user" /> {{ item.name }} &nbsp; <i class="el-icon-phone" /> {{ item.phone }}</p>
...
export default {
  data() { return {
    loading: false, errorMessage: '', id: '',
    formInline: { title: '', date: '' }, currentPage: 1, page: { page: 1, count: 10 },
    tableData: [], activeName: 'first', total: 0,
  }},
  created() { this.id = localStorage.getItem("userID"); this.getData(); },
  methods: {
    handleClick() { this.page.page = 1; if (this.activeName === 'first') this.getData(); else this.getFoundData(); },
    handleReset() { this.formInline = { title: '', date: '' }; this.page = { page: 1, count: 10 }; if (this.activeName === 'first') this.getData(); else this.getFoundData(); },
    handleDelete(row) { request.get("/main/deleteContact", { params: { id: row.id } }).then(() => { this.$notify({ title: '成功', message: '删除成功', type: 'success', offset: 55 }); if (this.activeName === 'first') this.getData(); else this.getFoundData(); }); },
    handleSizeChange(val) { this.page.count = val; if (this.activeName === 'first') this.getData(); else this.getFoundData(); },
    handleCurrentChange(val) { this.page.page = val; if (this.activeName === 'first') this.getData(); else this.getFoundData(); },
    getData() {
      ...
      request.get("/main/getContact", { params: { page: this.page.page, count: this.page.count, title: this.formInline.title, contactTime: this.formInline.date, itemsUserID: this.id } }).then(res => {
        ...
        const userPromises = data.map((item, i) => { item.itemPhoto = `/main/download?name=${item.itemPhoto}`; return request.get("/main/getUser", { params: { id: item.userID } }).then(r => { data[i].name = r.data.data.rows[0].name; data[i].phone = r.data.data.rows[0].phone; }).catch(() => { data[i].name = '未知'; data[i].phone = '未知'; }); });
    ...
    getFoundData() {
      ...
      request.get("/main/getFoundContact", { params: { page: this.page.page, count: this.page.count, title: this.formInline.title, contactTime: this.formInline.date, itemsUserID: this.id } }).then(res => {
        this.total = res.data.data.total; var data = res.data.data.rows;
        if (data.length < 1 && this.page.page > 1) { this.page.page -= 1; this.getFoundData(); return; }
        const userPromises = data.map((item, i) => { item.itemPhoto = `/main/download?name=${item.itemPhoto}`; return request.get("/main/getUser", { params: { id: item.userID } }).then(r => { data[i].name = r.data.data.rows[0].name; data[i].phone = r.data.data.rows[0].phone; }).catch(() => { data[i].name = '未知'; data[i].phone = '未知'; }); });
        Promise.all(userPromises).then(() => { this.tableData = data; this.loading = false; });
      }).catch(() => { this.tableData = []; this.total = 0; this.errorMessage = '数据加载失败'; this.loading = false; });
    }
...
<style scoped>
.contact-me { max-width: 1000px; margin: 0 auto; padding: 0 20px 40px; }
.page-banner { border-radius: 16px; padding: 40px 48px; margin-bottom: 24px; color: #fff; position: relative; overflow: hidden; }
.banner-contactme { background: linear-gradient(135deg, #409EFF 0%, #2d6cdf 100%); }
...
.result-count { color: #909399; font-size: 14px; } .result-count b { color: #409EFF; font-weight: 600; }
