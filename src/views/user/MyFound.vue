<template>
  <div class="my-found">
    <div class="page-banner banner-myfound">
      <div class="banner-content">
        <h1 class="banner-title">我的招领</h1>
        <p class="banner-subtitle">管理你发布的招领信息</p>
      </div>
    </div>

    <div class="search-section">
      <el-form :inline="true" :model="formInline" class="search-form">
        <el-form-item><el-input v-model="formInline.title" placeholder="搜索标题..." clearable prefix-icon="el-icon-search" size="medium" class="search-input" @input="handleSearchInput" /></el-form-item>
    ...
    <div class="toolbar">
      ...
      <el-button type="success" icon="el-icon-plus" size="small" round @click="dialogFormVisible = true">发布新招领</el-button>
    </div>
    ...
    <div v-loading="loading" element-loading-text="正在加载..." class="card-grid-wrapper">
      ...
      <div v-else-if="!loading && tableData.length === 0" class="empty-box"><i class="el-icon-folder-opened" /><p>还没有发布过招领</p><span>点击上方按钮发布第一条吧</span></div>
      <div v-else class="card-grid">
        <div v-for="item in tableData" :key="item.id" class="my-card">
          <div class="card-image" @click="handelView(item)">
            ...
            <span class="card-status" :class="{ 'status-audit': item.status==='审核中', 'status-reject': item.status==='未通过', 'status-ok': item.status==='已认领' }">{{ item.status }}</span>
          </div>
          ...
          <div class="card-footer">
            ...
            <el-popconfirm title="确定要删除该条招领信息吗？" @confirm="handleDelete(item)">
    ...
    <div class="pagination-wrap"><el-pagination :current-page="currentPage" :page-sizes="[8, 16, 24, 32]" :page-size="page.count" layout="total, sizes, prev, pager, next, jumper" :total="total" background @size-change="handleSizeChange" @current-change="handleCurrentChange" /></div>

    <el-dialog title="添加招领" :visible.sync="dialogFormVisible" width="500px" @close="handleClose">
      <el-form ref="addForm" :model="addForm" :rules="rules"><el-form-item label="物品图片" :label-width="formLabelWidth"><el-upload class="avatar-uploader" action="/main/upload" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload"><img v-if="imageUrl" :src="imageUrl" class="avatar"><i v-else class="el-icon-plus avatar-uploader-icon" /></el-upload></el-form-item>
        ...
        <el-form-item label="手机号" :label-width="formLabelWidth" prop="phone"><el-input v-model="addForm.phone" /></el-form-item></el-form>
      <div slot="footer"><el-button @click="handleClose">取消</el-button><el-button type="primary" @click="addFound('addForm')">确定</el-button></div>
    </el-dialog>

    <el-dialog title="修改招领信息" :visible.sync="dialogUpdateForm" width="500px" @close="handleClose">
      <el-form ref="updateForm" :model="updateForm" :rules="rules"><el-form-item label="物品图片" :label-width="formLabelWidth"><el-upload class="avatar-uploader" action="/main/upload" :show-file-list="false" :on-success="handleUpdateSuccess" :before-upload="beforeAvatarUpload"><img v-if="imageUrl" :src="imageUrl" class="avatar"><i v-else class="el-icon-plus avatar-uploader-icon" /></el-upload></el-form-item>
        ...
        <el-form-item v-if="updateForm.statusID != 8 && updateForm.statusID != 7" label="状态" :label-width="formLabelWidth" prop="statusID"><el-select v-model="updateForm.statusID" style="width:100%"><el-option v-for="item in statusList" :key="item.id" :label="item.status" :value="item.id" /></el-select></el-form-item></el-form>
      <div slot="footer"><el-button @click="handleClose">取消</el-button><el-button type="primary" @click="editFound('updateForm')">确定</el-button></div>
    </el-dialog>

    <el-dialog title="详细信息" :visible.sync="dialogShowView" width="500px"><el-form :model="showView"><el-form-item label="物品图片" :label-width="formLabelWidth"><img :src="showView.itemPhoto" class="avatar"></el-form-item>
      ...
      <el-form-item label="状态" :label-width="formLabelWidth"><el-input v-model="showView.status" :disabled="true" /></el-form-item></el-form>
      <div slot="footer"><el-button type="primary" @click="dialogShowView = false">确定</el-button></div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    editFound(form) { this.$refs[form].validate(v => { if (v) { request.post("/main/editFound", JSON.stringify(this.updateForm), { headers: { "Content-Type": "application/json" } }).then(() => { this.$notify({ title: '成功', message: '修改成功', type: 'success', offset: 55 }); this.dialogUpdateForm = false; this.getData(); }); } }); },
    handleRefresh(row) { request.post("/main/editFound", JSON.stringify({ id: row.id, statusID: 7 }), { headers: { "Content-Type": "application/json" } }).then(() => { this.$notify({ title: '成功', message: '已重新提交审核', type: 'success', offset: 55 }); this.getData(); }); },
    showEdit(row) { request.get("/main/getFound", { params: { id: row.id } }).then(res => { this.updateForm = res.data.data.rows[0]; this.imageUrl = `/main/download?name=${this.updateForm.itemPhoto}`; this.dialogUpdateForm = true; }); },
    ...
    addFound(form) { this.$refs[form].validate(v => { if (v) { this.addForm.userID = this.id; request.post("/main/addFound", JSON.stringify(this.addForm), { headers: { "Content-Type": "application/json" } }).then(() => { this.$notify({ title: '成功', message: '添加成功', type: 'success', offset: 55 }); this.dialogFormVisible = false; this.getData(); }); } }); },
    ...
    handleDelete(row) { request.get("/main/deleteFound", { params: { id: row.id } }).then(() => { this.$notify({ title: '成功', message: '删除成功', type: 'success', offset: 55 }); this.getData(); }); },
    ...
    getData() {
      ...
      request.get("/main/getFoundStatus").then(res => { this.statusList = res.data.data; });
    }
  },
}
</script>
...
<style scoped>
.my-found { max-width: 1300px; margin: 0 auto; padding: 0 20px 40px; }
.page-banner { border-radius: 16px; padding: 40px 48px; margin-bottom: 24px; color: #fff; position: relative; overflow: hidden; }
.banner-myfound { background: linear-gradient(135deg, #67C23A 0%, #3d8b37 100%); }
